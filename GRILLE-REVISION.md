# Grille de révision — enseignante

Points de vérification à parcourir en lisant la demande de fusion.
**Ceci n'est pas la grille d'évaluation** : aucune note, aucun jugement ne
figure ici. La grille d'évaluation reste dans Word et se remet dans StudiUM.

Ce fichier est visible des étudiants, et c'est voulu : savoir ce qui sera
vérifié fait partie de l'apprentissage.

## Question et concepts

- La question permet-elle d'identifier les concepts sans ambiguïté ?
- La pertinence clinique est-elle argumentée, ou seulement affirmée ?
- Le découpage en concepts est-il juste — ni concept superflu qui restreint
  indûment, ni concept manquant qui élargit trop ?
- La portée déclarée de chaque concept correspond-elle à ce qui est cherché
  ensuite ?

## Vocabulaire contrôlé

- Chaque descripteur existe-t-il dans la version du thésaurus indiquée ?
- Les descripteurs sont-ils ceux de la base déclarée — MeSH, CINAHL Headings,
  Emtree ne sont pas interchangeables ?
- L'option Explode est-elle justifiée ? Appliquée systématiquement sans
  réflexion, ou absente là où l'arborescence l'exigeait ?
- Manque-t-il un descripteur évident pour l'un des concepts ?

## Vocabulaire libre

- Les troncatures sont-elles placées au bon endroit — ni trop courtes, ce qui
  génère du bruit, ni trop longues, ce qui perd des variantes ?
- Les synonymes couvrent-ils les variantes orthographiques et les usages
  britannique et américain ?
- Les champs interrogés sont-ils appropriés — titre, résumé et mots-clés
  d'auteur plutôt que tous les champs ?

## Équations

- L'équation reproduite correspond-elle aux termes déclarés plus haut ?
- OU à l'intérieur d'un concept, ET entre les concepts ?
- Les parenthèses ferment-elles ce qu'il faut ?
- Les opérateurs d'adjacence sont-ils utilisés à bon escient ?

## Filtres et résultats

- Les filtres sont-ils justifiés, et ne risquent-ils pas d'exclure des études
  pertinentes ?
- Le nombre de résultats est-il plausible compte tenu de la stratégie ?
- La date d'exécution figure-t-elle pour chaque base ?

## Reproductibilité

- Une personne extérieure pourrait-elle réexécuter cette recherche telle
  quelle ?
- Le document se comprend-il sans le contexte du cours ? C'est une condition
  d'admissibilité au dépôt dans Papyrus.
